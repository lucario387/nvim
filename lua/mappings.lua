...
M.general = function()
  ...
  set("n", "<C-`>", function()
    ...
    for _, winnr in ipairs(winnrs) do
      if vim.api.nvim_buf_get_option(vim.api.nvim_win_get_buf(winnr), "buftype") == "terminal" then
        no_term = false
        term_winnr = winnr
        term_bufnr = vim.api.nvim_win_get_buf(winnr)
        break
      end
    end
    if no_term then
      ...
      vim.cmd("confirm bd! " .. term_bufnr)
      -- term_winnr = nil
      -- term_bufnr = nil
    end

  end, {desc = "Open/Close terminal of current tab"})
  set("t", "<C-x>", "<C-\\><C-n>", {desc = "Exit terminal mode", silent = true})
  set("t", "<C-`>", function()
    local winnrs = vim.api.nvim_tabpage_list_wins(0)
    local no_term = true
    local term_winnr
...
M.jdtls = function(bufnr)
	...
	set("x", "<leader>jv", function()
		require("jdtls").extract_variable(true)
	end, { desc = "Extract variable", buffer = bufnr })
	set("x", "<leader>jc", function()
		require("jdtls").extract_constant(true)
	end, { desc = "Extract constant", buffer = bufnr })
	set("x", "<leader>jm", function()
